...
广告放在文章**最后**，紧跟在末尾分割线之后。广告内容**必须和本文话题相关**，动态生成，不能照搬模板。
...
> 如果这篇文章涉及的 [文章核心话题] 你想系统深入，可以看看我之前出版的[《动手学 AutoML：从 NAS 到大语言模型优化实战》](https://item.jd.com/14945889.html)，[一句话说明书里哪个具体内容和本文相关，诚实，不夸大]。
>
> ![动手学AutoML书籍封面](https://github.com/marsggbo/marsggbo.github.io/blob/master/assets/img/book_cover_automl.png?raw=true)
```

**如何写广告文案（核心原则：点到为止，不吹嘘）**：
1. 开头固定格式：`如果这篇文章涉及的 [话题] 你想系统深入，`
2. 引出书名：《动手学 AutoML：从 NAS 到大语言模型优化实战》
3. 一句话说明书里哪个具体章节/内容和本文直接相关
4. **禁止**：不写"强烈推荐"、"必读"、"干货满满"等空话；不夸大书的范围；不说"适合所有人"

**不同话题的广告示例**（仅供参考，实际按文章内容调整）：

| 文章话题 | 广告关联句参考 |
|---------|-----------|
| LLM 推理优化（KV Cache、量化、加速） | "书里有专章讲 LLM 推理效率和参数高效微调，正好和本文的工程背景有关联" |
| NAS / 神经架构搜索 | "书里 NAS 部分从原理到 one-shot 实现都有，和本文讲的方法一脉相承" |
| AutoML / 超参优化 | "本书核心就是讲这块，从贝叶斯优化到进化算法都有，感兴趣可以翻翻" |
| MoE / 稀疏模型 | "书里讲到了 NAS 在大模型上的延伸，和 MoE 的架构搜索思路有些交叉" |
| Diffusion / 生成模型效率 | "书里虽然不直接讲 diffusion，但 LLM 效率优化那章和本文的加速思路有些呼应" |

**何时跳过广告**：
- 用户明确说"不要广告"、"去掉广告"、"no book ad"、"skip ad" 等
- 文章话题与书完全无关（如纯系统、非 AI 话题）

**何时保留广告**：用户未提广告，且文章话题和 AutoML/LLM/NAS/推理优化有任何关联时，默认加上。
