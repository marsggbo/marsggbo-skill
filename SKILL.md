...
- **结尾贴"一点 take"/"我的 takeaway"这类自称视角的标签，正文堆砌"我认为""我的理解是"**（见下方硬性规则）
- **章节标题写成互不相关的名词短语，或暴露"所以接下来我们来看看 XX"这类写作心路历程**（见下方硬性规则）
...
1. **类别统称**：SpeechLLM、Agent、MoE、omni model——最容易被误当成具体产品名，要明确点出"这不是某个模型，是一类架构的统称"，再给几个实例
2. **评测指标**：CER / WER / B-CER / Recall@K / FAsil——要说清**算的是什么、越高越好还是越低越好**
3. **方法/算法缩写**：GRPO、LoRA、SFT、GLCLAP、CLAP、RAG、DPO、EPLB——一句话说清干什么，能类比就类比（"CLAP 可以理解成 CLIP 的音频版"）
4. **领域行话**：utterance、enrollment、contextual biasing、bookkeeping——外行看不出这是术语，更需要解释

#### 解释要顺着行文长出来，不能像在念名词表（同等重要）

**这条和"必须解释"同等重要**：解释术语不等于插入一段词典条目。读者要的是顺着读下去自然就懂了，不是中途被拽去看一条定义。

```
❌ 错误（教科书腔，"先解释一个词"这种元叙述最出戏）：
先解释一个词，后面全篇都要用：SpeechLLM。它不是某个具体模型的名字，而是
一类架构的统称——把音频编码器接到自回归语言模型主干前面……

❌ 错误（括号里塞一整段定义，把句子撑断）：
Qwen3-ASR-1.7B 在 AIShell-1 上 CER 1.58%（CER = Character Error Rate 字错率，
指改动多少个字才能把识别结果变成参考答案占参考长度的比例，替换、删除、插入
都算，越低越好。中文按字算所以用 CER，英文按词算所以用 WER）——这个精度……

✅ 正确（术语作为叙事的一部分被带出来）：
转机出现在 ASR 换代之后。这个领域大致走过四代……现在到了第五代：把音频编码器
直接接到一个 LLM 前面，音频编码成 embedding 当 token 喂进去，由 LLM 自回归地
把文字解码出来。这类模型现在一般叫 SpeechLLM，注意这不是某个模型的名字，而是
这一类架构的统称，Qwen3-ASR、Kimi-Audio、Step-Audio 2 都属于这一类。

✅ 正确（指标顺着句子讲完，不打断）：
Qwen3-ASR-1.7B 在 AIShell-1 上 CER 1.58%、LibriSpeech 上 WER 1.61%——中文按字
算错误率所以用 CER，英文按词算所以用 WER，都是"要改动多少个字/词才能把识别结果
变成标准答案"，越低越好。1.58% 这个精度已经好到没什么可优化的了。

✅ 正确（顺手带出，还把指标的动机说了）：
这里的指标是 B-CER，前面那个 B 是 biased，只统计实体 token 上的错误，句子其他
部分错了不算。为什么要这么算？因为垂域场景在乎的从来不是整句准确率——虚词错一个
无所谓，药品名错一个就是事故。
```

具体做法：

- **不要写元叙述**："先解释一个词"、"这里插入一个概念"、"需要说明的是"——直接讲这个词是什么就行
- **不要在括号里塞一整段**。括号适合一句话（"注册音频，虚线表示可选"），塞进三行定义就该拆出来当正文
- **优先"讲故事时顺手带出"**，而不是"停下来定义"。比如讲技术演进讲到第五代，SpeechLLM 就自然出现了；讲某个指标为什么这么设计，定义就顺带交代完了
- **能类比就类比**，一句"CLAP 可以理解成 CLIP 的音频版"胜过三行公式描述
- **越常见的词越简短**。RAG 对 LLM 读者只需要一句"还是熟悉的那个检索增强生成，只是检索的不是文档而是热词条目"，不需要展开全称和历史

**自查方法**：把补上去的解释**当普通正文读一遍**，如果读起来像"作者停下来给我上了一课"，就是写死板了，改成叙事的一部分。另外自己扫一遍全文，凡是英文缩写和 `@` / `-` 连接的指标名，检查它第一次出现的那句话有没有交代——**跨领域时尤其要查**（给 LLM 读者写 ASR / TTS 解读，读者的领域直觉完全不起作用）。
...
### 结尾用"总结"/"小结"，不用"一点 take"（硬性规则）

文章收尾如果要收束观点，标题写**总结**或**小结**这类中性表述。**不要写"一点 take"、"我的 takeaway"、"个人感受"这类强调"以下是我个人主观发言"的标签**——这本质上和开场自称身份是同一类问题：给观点贴一个"这是我说的"的标签，并不会让观点更可信。
...
正文不要堆砌"我认为"、"我的理解是"、"我看"、"在我看来"这类第一人称限定语。技术判断直接用陈述句给出——文章本身就是作者视角，不需要每句话前面加一个"我觉得"去提醒读者这是主观的。

个人具体经历仍然可以用第一人称叙述（"自己做 ExpertFlow 时就踩过这个坑"这种是提供信息），但**评价性、结论性的句子应该直接陈述**，不需要"我认为"起头（那是在索取认同，不是提供信息，和"禁止自我标签式开场"是同一个原则）。

```
❌ 错误：我的理解是，CoT 在这里更大的价值不是提升精度，而是提供可解释性。
✅ 正确：CoT 在这里更大的价值不是提升精度，而是提供可解释性。

❌ 错误：我觉得这篇论文的核心创新其实是工程和数据层面的取舍。
✅ 正确：这篇论文的核心创新，与其说是架构，不如说是工程和数据层面的取舍。
```

### 章节标题要体现逻辑递进，不要写成心路历程（硬性规则）

数字编号章节（`## 1.` `## 2.` ...）不能是互不相关的平行名词列表，标题本身要让读者一眼看出"从哪来、到哪去"——背景→矛盾→方案→细节→效果→总结这条主线，应该体现在标题措辞的递进或因果关系里，不能靠读者自己脑补。这条不只适用于源码解读（见下方 E.1），适用于所有多章节文章，尤其是论文解读。

但递进关系**不等于把写作意图暴露出来**。不要在标题或正文里写"所以我们来看看 XX 是怎么做的"、"接下来分析一下 XX"这类交代写作动机的句子——那是心路历程，不是内容。
...
## 4. 训练流程
## 5. 实验结果
   → 四节是并列陈述，标题没有传达"架构决定了训练要怎么分阶段""训练方式
     决定了效果为什么好"这类因果关系，读者只能自己脑补

✅ 正确（标题本身带出递进/因果关系）：
## 2. 不做语音分离，直接把参考语音编码进同一套表征
## 3. 训练分四阶段，从纯 ASR 一路"加码"到 TS-ASR + CoT
## 4. 拒识能力是训出来的，不是靠阈值加出来的
## 5. 多人 SOTA、单人不掉分：实验数据说明了什么
```

**自查方法**：把全文的章节标题单独抽出来连起来读一遍，不看正文——能不能读出一条完整的逻辑线（是什么→为什么这样设计→效果如何）？如果读出来是一堆互不相关的名词短语，说明标题只在分类，没在讲道理。
...
但**解释的写法同样是硬要求**：不要写"先解释一个词"这种元叙述，不要在括号里塞一整段定义，要让术语顺着行文自然被带出来（比如讲技术演进讲到最新一代，这代的名字就自然出现了；讲指标为什么这么设计，定义就顺带交代完了）。详细的 ❌/✅ 对照见上方「解释要顺着行文长出来，不能像在念名词表」。
...
#### 上传前必须检查 Jekyll front matter（硬性规则）

跑上传脚本之前，先 Read 一下目标 `.md` 文件的前 10 行，确认：

1. **front matter 块确实存在**：文件最开头是 `---\nlayout: post\ntitle: "..."\ndate: YYYY-MM-DD\ntags: [...]\n---`。缺了这个块的文章上传后会在线上渲染成没有样式的裸文本（Jekyll 对 `_posts` 里没有 front matter 的文件不会走模板/layout 处理）——真实踩过的坑：CocktailASR-1 那篇文章第一次上传时整段 front matter 都没写，用户看到线上页面"乱七八糟"才发现。
2. **YAML 本身合法**：标题里如果用引号强调某个词（这个写作风格里常见），内部引号必须是中文弯引号 `“”`，不能是英文直引号 `"`——`title: "..."` 值内部出现一个直引号会破坏 YAML 解析，但在普通 Markdown 编辑器里看起来完全正常，本地怎么看都不会报错。拿不准格式时对照一篇已知正常的历史文章（如 `_posts/2026-09-10-20260910-deepseek-v41-flash.md`）。

**原因**：本地 `.md` 文件不管有没有 front matter、引号对不对，用普通 Markdown 查看器打开都长得一样，问题只有 Jekyll 真正构建时才会暴露，所以不能假设"本地看着没问题"就等于"上线也没问题"。
...
上传成功后，用 `gh api repos/<owner>/<repo>/contents/_posts/<file>.md` 把文件从 GitHub 读回来，确认 front matter 完整无损（而不是默认脚本"跑完没报错"就等于"传对了"），再告知用户文章链接（格式：`https://marsggbo.github.io/blog/<year>/<title>/`）。
...
- 架构/方法段落怎么写，见下面 D.1-D.3

#### D.1 架构/方法段落必须自上而下叙事，背景先行（硬性规则）

写论文的架构/方法部分，不能一上来就说"A 变成一个 embedding 喂给 LM"——那是把最终结果当开场白，读者没经过"为什么难""怎么解决"这两步就直接看到答案，文章会显得单薄、像在念模块清单。

**顺序固定为：开场只点题，不展开机制 → 交代背景困难 → 再回来展开怎么解决。**

1. **开场只点题，不展开机制**：在前言里用一句话说出这篇论文最特别的设计选择（比如"把原本分开的 N 个模块合并成一个模型"），当钩子，但不要在这里就讲"具体怎么合并的"。
2. **背景段落交代清楚"分开做"曾经是什么样、"合起来"为什么难**：常见的难点包括——误差沿流水线累积（上一模块的缺陷原样传给下一模块）、一个模块的训练目标对另一个任务的副作用（比如为区分身份训练出的信号，注入到别的任务里就变成噪声）、分开做时压根不存在的能力（比如没有模块专门负责"拒绝回答"）。这一段没写透，后面"合并"的特别之处就立不住，架构段落会显得是无缘由的炫技。
3. **背景铺完，才展开解决方案**——这时候"怎么解决"的机制描述才有落点：读者能看出这个设计具体化解了上一步说的哪个困难，不是自己凑巧长这样。

**自查方法**：删掉架构段落，只留前言点题句和背景困难段——读者能不能自己猜到"这篇论文大概会怎么解决"？如果背景段落只是空泛地说"这很难"，没具体到"错误会怎么累积""哪个信号会干扰哪个任务"，说明背景没交代到位。

#### D.2 架构描述必须讲清楚三件事，不能停在"模块名 + 一句话"（硬性规则）

架构/方法段落只说"A 模块的输出变成 embedding，喂给 B"是不够的，必须讲清楚：

1. **模块怎么接起来**：真实的 tensor 维度、每次交接时的形状变化，不能只写模块名字。例如"D2V2 编码器输出经过 `Linear(1280, 1280)` 和 `Linear(1280, 4096)` 两级映射，4096 正好是 Qwen3-8B 的隐藏维度"，而不是笼统地说"音频特征被映射到 LLM 的维度"。
2. **端到端的完整数据流**：输入是什么、输出是什么，一步步怎么走，不能跳步骤。
3. **每一种任务/模式各自的输入输出格式**：如果模型支持多种推理模式（标准推理 / CoT / 拒识场景 / 不同训练阶段），每一种模式的输入怎么拼、输出是什么样子，都要单独说清楚，不能只描述一种模式就当成全部——用表格列出「场景｜输入音频｜输入 prompt｜输出」通常比一段话堆下来更清楚。

**这三件事必须从开源代码里核实，不能凭对论文文字/二次摘要的印象转述**——踩过的坑：第一次用 WebFetch 转述某论文建模代码摘要时把 LLM 隐藏维度记成 3584，实际读 `config.json` 发现是 4096。涉及具体数字的地方，永远去读一手源码文件确认，不要相信二次转述。

#### D.3 顺序：讲透任务和输入输出之后，才谈训练、数据、附加设计（硬性规则）
...
对于第 3 类附加设计（比如一个 CoT 设计，量化收益很有限），**必须诚实说清楚论文自己给出的实际价值是什么**（可解释性、给下游任务提供辅助信号等），不能因为这个设计"听起来合理"就含糊地说它有用，却不说清楚它到底好在哪、代价是什么——数字不好看就老实摆数字，然后讲清楚论文自己承认的真实卖点是什么。
