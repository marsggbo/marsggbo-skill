...
**参考写法（根据文章话题选择，不照抄，每次换一种表达）**：

直接相关（NAS/LLM压缩/AutoML）：
```
写这篇的时候想到，我们团队把 NAS 和 AutoML 的相关研究整理成了
[《动手学 AutoML：从 NAS 到大语言模型优化实战》](https://item.jd.com/14945889.html)，
书里第 X 章就有 [具体内容] 的实战代码，感兴趣可以翻翻。
```

话题相邻（LLM推理效率/MoE/SSD）：
```
顺带一提，我们团队在做 LLM 效率优化方向，也出了本从 AutoML 视角看这类问题的书——
[《动手学 AutoML：从 NAS 到大语言模型优化实战》](https://item.jd.com/14945889.html)。
这篇讲的是推理端的工程实现，书更多是从架构搜索和压缩的角度切入，角度不同，
但目标都是让模型跑得更高效。
```

话题较远（vllm源码/系统调度/ASR等）：
```
扯句题外话——我们团队做 LLM 研究，也出了本 [《动手学 AutoML：从 NAS 到大语言模型优化实战》](https://item.jd.com/14945889.html)，
讲的是 AutoML、NAS 以及它们在 LLM 上的应用，和这篇的话题不直接重叠，
如果你对"怎么系统地优化大模型"这件事感兴趣，可以看看。
```
